...
export default function BlogPreview() {
  ...
  if (posts.length === 0) return <div className="text-[#8FA6BC] text-[14px]">Primele articole apar în curând.</div>;
  ...
  const Arrow = ({ dir, disabled }: { dir: 1 | -1; disabled: boolean }) => (
    <button
      ...
      disabled={disabled}
      className={`absolute top-1/2 -translate-y-1/2 ${dir === 1 ? 'right-0 sm:-right-3' : 'left-0 sm:-left-3'} z-10 grid place-items-center w-11 h-11 rounded-full bg-white text-[#07090f] shadow-[0_8px_24px_-8px_rgba(10,34,56,0.35)] ring-1 ring-black/[0.06] transition-opacity ${disabled ? 'opacity-0 pointer-events-none' : 'opacity-100 hover:bg-[#F5F8FB]'}`}
    >
      ...
      >
        {posts.map((p) => (
          <a key={p.slug} href={`/blog/${p.slug}`}
            ...
            <div className="flex items-center gap-2 text-[12px] text-[#8FA6BC] mb-2">
              {p.category && <span className="px-2 py-0.5 rounded-full bg-[#E1FB15]/30 text-[#07090f] font-semibold">{p.category}</span>}
              <span>{p.readMinutes} min</span>
            </div>
            <h3 className="text-[16px] font-bold text-[#07090f] tracking-[-0.02em] mb-1.5 leading-snug line-clamp-2">{p.title}</h3>
            <p className="text-[13.5px] text-[#46627A] leading-relaxed line-clamp-2">{p.description}</p>
            <p className="text-[11px] text-[#A8BED2] mt-3">{fmtDate(p.publishedAt)}</p>
          </a>
        ))}
      </div>
