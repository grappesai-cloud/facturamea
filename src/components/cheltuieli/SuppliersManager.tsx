  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Button onClick={() => setEditing({ ...empty })}><Plus className="w-4 h-4 mr-1" /> Furnizor nou</Button>
      </div>

      <Card>
        <CardContent className="p-0">
          {items.length === 0 ? (
            <p className="text-sm text-[#6B6B68] p-6 text-center">Niciun furnizor. Adaugă primul furnizor.</p>
          ) : (
            <ul className="divide-y divide-[#E8E8E4]">
              {items.map((s) => (
                <li key={s.id} className="flex items-center gap-3 px-4 py-3 hover:bg-[#FAFAF8] transition-colors">
                  ...
                  <button onClick={() => setEditing({ ...empty, ...s, cui: s.cui || '', country: s.country || 'Romania' } as any)} className="p-1.5 text-[#6B6B68] hover:text-[#0A0A0A]"><Pencil className="w-4 h-4" /></button>
                  <button onClick={() => remove(s.id)} className="p-1.5 text-[#A8A8A4] hover:text-[#B91C1C]"><Trash2 className="w-4 h-4" /></button>
                </li>
              ))}
            </ul>
          )}
      ...
      {editing && (
        <Card>
          <CardContent className="p-4 space-y-3">
            <h3 className="font-semibold text-[#0A0A0A]">{editing.id ? 'Editează furnizor' : 'Furnizor nou'}</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                <Label className="mb-1 block text-xs">CUI</Label>
                <div className="flex gap-1">
                  <Input value={editing.cui} onChange={(e) => setEditing({ ...editing, cui: e.target.value })} placeholder="RO12345678" />
                  <Button size="sm" variant="outline" onClick={lookupCui} disabled={looking || !editing.cui} title="Preia din ANAF">{looking ? <Loader2 className="w-4 h-4 animate-spin" /> : 'ANAF'}</Button>
                </div>
              </div>
              ...
              <div><Label className="mb-1 block text-xs">Reg. comerțului</Label><Input value={editing.regCom} onChange={(e) => setEditing({ ...editing, regCom: e.target.value })} placeholder="J40/..." /></div>
