    ...
    return (
      <Card>
        <CardContent className="p-8 text-center space-y-4">
          <div className="mx-auto w-16 h-16 rounded-full bg-[#E7F7EC] flex items-center justify-center">
            <Check className="w-8 h-8 text-[#15803D]" />
          </div>
          <div>
            <h2 className="text-[20px] font-bold text-[#0A0A0A]">Cheltuiala a fost salvată</h2>
            <p className="text-[15px] text-[#6B6B68] mt-1">O găsești în lista de cheltuieli.</p>
          </div>
          <div className="flex flex-wrap justify-center gap-2 pt-2">
            <Button size="lg" onClick={reset}><Camera className="w-5 h-5 mr-1" /> Scanează altă chitanță</Button>
            <a href="/app/cheltuieli"><Button size="lg" variant="outline">Vezi cheltuielile</Button></a>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <Card>
          <CardContent className="p-6 sm:p-8">
            <div className="text-center max-w-xl mx-auto">
              <div className="mx-auto w-14 h-14 rounded-2xl bg-[#FFF3E9] flex items-center justify-center mb-4">
                <FileText className="w-7 h-7 text-[#FF5C00]" />
              </div>
              <h2 className="text-[20px] font-bold text-[#0A0A0A]">Fă o poză la bon sau factură</h2>
              <p className="text-[15px] text-[#6B6B68] mt-2 leading-relaxed">
              ...
              {phase === 'scanning' ? (
                <div className="mt-6 flex flex-col items-center gap-3 py-4">
                  ...
                  <p className="text-[15px] font-semibold text-[#0A0A0A]">Se citește documentul...</p>
        ...
        <Card>
          <CardContent className="p-5 sm:p-6 space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-[18px] font-bold text-[#0A0A0A]">Verifică datele</h2>
                {fileName && <p className="text-[13px] text-[#8A8A85] mt-0.5 truncate max-w-[280px]">{fileName}</p>}
              </div>
              <Button size="sm" variant="ghost" onClick={reset} title="Reia"><RotateCcw className="w-4 h-4 mr-1" /> Reia</Button>
            </div>

            {note && <p className="text-[14px] text-[#15803D] bg-[#E7F7EC] rounded-xl px-3.5 py-2.5">{note}</p>}
            {error && <p className="text-[14px] text-[#B91C1C] bg-[#FDECEC] rounded-xl px-3.5 py-2.5">{error}</p>}

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <Label className="mb-1.5 block text-[14px]">Furnizor</Label>
                <Input className="h-12 text-[16px]" value={form.supplierName} onChange={(e) => set('supplierName', e.target.value)} placeholder="Numele furnizorului" />
              </div>
              <div>
                <Label className="mb-1.5 block text-[14px]">Număr document</Label>
                <Input className="h-12 text-[16px]" value={form.documentNumber} onChange={(e) => set('documentNumber', e.target.value)} placeholder="ex: 12345" />
              </div>
              <div>
                <Label className="mb-1.5 block text-[14px]">Data emiterii</Label>
                <Input className="h-12 text-[16px]" type="date" value={form.issueDate} onChange={(e) => set('issueDate', e.target.value)} />
              </div>
              <div>
                <Label className="mb-1.5 block text-[14px]">Categorie</Label>
                <Select className="h-12 text-[16px]" value={form.category} onChange={(e) => set('category', e.target.value)}>
                  {CATEGORIES.map((c) => <option key={c} value={c}>{CAT_LABELS[c]}</option>)}
                </Select>
              </div>
              <div>
                <Label className="mb-1.5 block text-[14px]">Total (RON)</Label>
                <Input className="h-12 text-[16px] font-semibold" type="number" step="any" value={form.total} onChange={(e) => set('total', e.target.value)} placeholder="0.00" />
              </div>
              <div>
                <Label className="mb-1.5 block text-[14px]">Bază fără TVA (RON)</Label>
                <Input className="h-12 text-[16px]" type="number" step="any" value={form.net} onChange={(e) => set('net', e.target.value)} placeholder="0.00" />
              </div>
              <div>
                <Label className="mb-1.5 block text-[14px]">TVA (RON)</Label>
                <Input className="h-12 text-[16px]" type="number" step="any" value={form.vat} onChange={(e) => set('vat', e.target.value)} placeholder="0.00" />
              </div>
            </div>

            <div className="flex flex-col sm:flex-row gap-2 pt-1">
              <Button size="lg" disabled={phase === 'saving'} onClick={save} className="w-full sm:w-auto">
                {phase === 'saving' ? <><Loader2 className="w-5 h-5 mr-1 animate-spin" /> Se salvează...</> : <><Check className="w-5 h-5 mr-1" /> Salvează cheltuiala</>}
              </Button>
              <Button size="lg" variant="outline" disabled={phase === 'saving'} onClick={reset} className="w-full sm:w-auto">Renunță</Button>
            </div>
          </CardContent>
        </Card>
      )}
