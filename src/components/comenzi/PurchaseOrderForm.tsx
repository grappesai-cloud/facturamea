          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div>
              ...
              {suppliers.length > 0 ? (
                ...
                <Input value={supplierName} onChange={(e) => setSupplierName(e.target.value)} placeholder="Nume furnizor" />
              )}
            </div>
            ...
            <div><Label className="mb-1 block text-xs">Livrare estimată</Label><Input type="date" value={expectedDate} onChange={(e) => setExpectedDate(e.target.value)} /></div>
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
            ...
            {lines.map((l, i) => (
              <div key={i} className="grid grid-cols-1 md:grid-cols-12 gap-2 items-end border-b border-[#F0F0EC] pb-2">
                ...
                <div className="md:col-span-2">
                  <Label className="mb-1 block text-[10px] uppercase tracking-wider text-[#8A8A85]">Cost unitar</Label>
          ...
          <div className="flex flex-wrap gap-2">
            <Button disabled={busy} onClick={() => submit('sent')}>{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Salvează și trimite'}</Button>
            <Button disabled={busy} variant="outline" onClick={() => submit('draft')}>Salvează ca ciornă</Button>
            <a href="/app/comenzi?tab=furnizori"><Button variant="ghost" type="button">Renunță</Button></a>
          </div>
        </CardContent>
      </Card>
    </div>
