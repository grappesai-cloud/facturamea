...
export default function ImportWizard() {
  ...
  return (
    <div className="bg-white border border-[#E8E8E4] rounded-2xl overflow-hidden">
      {/* Stepper header */}
      <div className="flex items-center gap-2 px-5 py-3.5 border-b border-[#F0F0EC] text-[12px]">
      ...
      <div className="p-5">
        {error && (
          <div className="mb-4 flex items-start gap-2 px-3.5 py-2.5 bg-[#FDECEC] border border-[#F5C2C2] rounded-xl text-[13px] text-[#B91C1C]">
        ...
        {step === 'setup' && (
          <div className="space-y-6">
            <div>
              <p className="text-[12px] font-semibold uppercase tracking-[0.1em] text-[#8A8A85] mb-2.5">
                1. De unde imporți?
              </p>
                ...
                {SOURCES.map((s) => (
                  <button
                    ...
                    className={`text-left p-3.5 rounded-xl border transition-colors ${
                      source === s.value
                        ? 'border-[#FF5C00] bg-[#FFF6F0]'
                        : 'border-[#E8E8E4] hover:border-[#A8A8A4]'
                    }`}
                  >
                    <span className="block text-[13px] font-semibold text-[#0A0A0A]">{s.label}</span>
                    <span className="block text-[11px] text-[#6B6B68] mt-0.5">{s.note}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-3 gap-2.5">
                {ENTITIES.map((e) => (
                  ...
                  >
                    <span className="block text-[13px] font-semibold text-[#0A0A0A]">{e.label}</span>
                    <span className="block text-[11px] text-[#6B6B68] mt-0.5">{e.note}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <label className="flex flex-col items-center justify-center gap-2 px-4 py-8 border-2 border-dashed border-[#E8E8E4] rounded-xl cursor-pointer hover:border-[#FF5C00] hover:bg-[#FFFBF8] transition-colors">
                <UploadCloud className="w-7 h-7 text-[#8A8A85]" />
                {file ? (
                  ...
                  <>
                    ...
                    <span className="text-[11px] text-[#8A8A85]">Acceptăm .csv și .xlsx (max. 15 MB)</span>
                  </>
                )}
                <input
                  ref={fileRef}
        ...
        {step === 'mapping' && preview && (
          <div className="space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {preview.truncated && (
                <span className="text-[11px] text-[#B45309] bg-[#FFF7E6] px-2 py-0.5 rounded-md">
            ...
            <div className="border border-[#E8E8E4] rounded-xl overflow-hidden">
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="text-left text-[11px] uppercase tracking-wider text-[#8A8A85] bg-[#FAFAF8] border-b border-[#F0F0EC]">
                ...
                <tbody>
                  {preview.headers.map((h) => (
                    <tr key={h} className="border-b border-[#F6F6F2] last:border-0">
                      <td className="px-4 py-2.5 font-medium text-[#0A0A0A] truncate max-w-[260px]">{h}</td>
            ...
            {/* Sample preview */}
            <div>
              <p className="text-[12px] font-semibold uppercase tracking-[0.1em] text-[#8A8A85] mb-2">
                Previzualizare (primele {preview.sample.length} rânduri)
              </p>
              <div className="border border-[#E8E8E4] rounded-xl overflow-x-auto">
                <table className="w-full text-[12px] whitespace-nowrap">
                  <thead>
                    <tr className="text-left text-[10px] uppercase tracking-wider text-[#8A8A85] bg-[#FAFAF8] border-b border-[#F0F0EC]">
                      {preview.headers.map((h) => (
                  ...
                  <tbody>
                    {preview.sample.map((row, i) => (
                      <tr key={i} className="border-b border-[#F6F6F2] last:border-0">
                        {preview.headers.map((h) => (
                          <td key={h} className="px-3 py-2 text-[#3D3D3A] truncate max-w-[180px]">
                            {row[h]}
                          </td>
                        ))}
                      </tr>
            ...
            <div className="flex items-center justify-between">
              ...
              <Button onClick={runCommit} disabled={busy || missingRequired.length > 0}>
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-3">
              {result.importedRows > 0 ? (
                ...
                <AlertTriangle className="w-9 h-9 text-[#B45309]" />
              )}
              <div>
            ...
            {result.truncated && (
              <p className="text-[12px] text-[#B45309] bg-[#FFF7E6] px-3 py-2 rounded-xl">
                Fișierul depășea 5000 de rânduri. Au fost procesate doar primele 5000.
              </p>
            )}

            {result.errors.length > 0 && (
              <div className="border border-[#F5C2C2] rounded-xl overflow-hidden">
                <p className="px-4 py-2 text-[11px] uppercase tracking-wider text-[#B91C1C] bg-[#FDECEC] font-semibold">
                  Erori ({result.errorRows})
                </p>
                <div className="max-h-56 overflow-y-auto divide-y divide-[#F6F6F2]">
                  {result.errors.map((e, i) => (
                    <p key={i} className="px-4 py-1.5 text-[12px] text-[#3D3D3A]">
                      <span className="font-mono text-[#8A8A85]">Rând {e.row}:</span> {e.message}
                    </p>
                  ))}
                </div>
              </div>
      ...
      <span
        className={`w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-bold ${
            ...
            : active
              ...
              : 'bg-[#F0F0EC] text-[#8A8A85]'
        }`}
      >
        {done ? '✓' : n}
      </span>
      <span className={`${active ? 'text-[#0A0A0A] font-semibold' : 'text-[#8A8A85]'}`}>{label}</span>
    </span>
  );
}
