      ...
      <Card className="max-w-md mx-auto bg-white/5 border-0 shadow-none rounded-2xl">
        <CardContent className="p-6 space-y-3">
          <div className="text-center">
            <p className="text-xs uppercase tracking-[0.12em] text-[#8FA6BC]">{receipt.fiscalStatus === 'printed' ? 'Bon fiscal' : 'Bon'}</p>
            <p className="text-lg font-bold font-mono text-white">{receipt.fiscalReceiptNumber || receipt.receiptNumber}</p>
          </div>

          {fiscalBusy && (
            <div className="flex items-center justify-center gap-2 text-sm text-[#A8BED2]">
          ...
          <div className="border-t border-dashed border-white/10 pt-3 space-y-1.5 text-sm">
            {receipt.lines.map((l, i) => (
              <div key={i} className="flex justify-between gap-2">
                <span className="text-[#A8BED2] truncate">{l.quantity} × {l.name}</span>
          ...
          <div className="border-t border-dashed border-white/10 pt-3 space-y-1 text-sm">
            <div className="flex justify-between"><span className="text-[#A8BED2]">Subtotal</span><span className="text-white tabular-nums">{ron(receipt.subtotalCents)}</span></div>
            <div className="flex justify-between"><span className="text-[#A8BED2]">TVA</span><span className="text-white tabular-nums">{ron(receipt.vatCents)}</span></div>
            ...
            <p className="text-xs text-[#A8BED2] pt-1">Plată: {receipt.paymentMethod === 'cash' ? 'Numerar' : receipt.paymentMethod === 'card' ? 'Card' : 'Mixt'}</p>
          </div>
          <div className="flex gap-2 pt-2">
            <Button className="flex-1 rounded-full bg-[#E1FB15] text-[#07090f] font-bold hover:bg-[#D2EA0E]" onClick={() => setReceipt(null)}>Vânzare nouă</Button>
      ...
      <div className="space-y-3">
        ...
        <Card className="bg-white/5 border-0 shadow-none rounded-2xl">
          <CardContent className="p-4 space-y-3">
            ...
            {cart.length === 0 ? (
              <p className="text-sm text-[#A8BED2] py-4 text-center">Coșul este gol. Atinge un produs.</p>
            ) : (
            ...
            <div className="border-t border-white/10 pt-3 space-y-1 text-sm">
              <div className="flex justify-between"><span className="text-[#A8BED2]">Subtotal</span><span className="text-white tabular-nums">{ron(totals.subtotal)}</span></div>
              <div className="flex justify-between"><span className="text-[#A8BED2]">TVA</span><span className="text-white tabular-nums">{ron(totals.vat)}</span></div>
              <div className="flex justify-between items-baseline"><span className="text-[#A8BED2]">Total</span><span className="text-[24px] sm:text-[28px] font-bold tracking-[-0.02em] tabular-nums text-white">{ron(totals.total)}</span></div>
            </div>
            ...
            <Button className="w-full rounded-full bg-[#E1FB15] text-[#07090f] font-bold hover:bg-[#D2EA0E]" disabled={busy || cart.length === 0} onClick={checkout}>
