...
export default function RegisterForm({ locale = 'ro' }: { locale?: Locale } = {}) {
  ...
  if (done) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-[26px] font-bold tracking-[-0.02em] text-[#0A0A0A] leading-tight">Verifică-ți emailul</h1>
          <p className="text-[13px] text-[#6B6B68] mt-1.5">
            Ți-am trimis un link de confirmare la <span className="font-semibold text-[#0A0A0A]">{email}</span>.
            Apasă pe el ca să-ți activezi contul, apoi autentifică-te.
          </p>
        </div>
        <a href="/auth/login" className="block w-full text-center py-3 bg-[#FF5C00] hover:bg-[#E04E00] text-white font-semibold rounded-xl text-[14px] transition-colors">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <h1 className="text-[26px] font-bold tracking-[-0.02em] text-[#0A0A0A] leading-tight">Creează cont</h1>
        <p className="text-[13px] text-[#6B6B68] mt-1.5">Pasul {step} din 2 · facturează în câteva minute</p>
      </div>
      ...
      <div className="flex gap-1.5">
        {[1, 2].map((s) => (
          <div key={s} className={`h-[3px] flex-1 rounded-full transition-all duration-300 ${s <= step ? 'bg-[#FF5C00]' : 'bg-[#E8E8E4]'}`} />
        ))}
      </div>
      ...
      {step === 1 && (
        <div className="space-y-4">
          <p className="text-[12px] uppercase tracking-[0.16em] text-[#6B6B68] font-medium">Firma ta</p>
          <div>
            <label className={labelCls}>CUI / CIF <span className="text-[#6B6B68]">(preia datele de la ANAF)</span></label>
            <input value={cui} onChange={(e) => setCui(e.target.value)} onBlur={lookupCui} className={inputCls} placeholder="ex. RO12345678" />
            {cuiHint && (
              <p className={`text-[11px] mt-1.5 ${cuiState === 'found' ? 'text-[#15803D]' : cuiState === 'loading' ? 'text-[#6B6B68]' : 'text-[#B45309]'}`}>{cuiHint}</p>
            )}
          </div>
          <div>
            <label className={labelCls}>Denumire firmă <span className="text-[#FF5C00]">*</span></label>
      ...
      {step === 2 && (
        <div className="space-y-4">
          <p className="text-[12px] uppercase tracking-[0.16em] text-[#6B6B68] font-medium">Contul tău</p>
          <div>
            <label className={labelCls}>Nume complet <span className="text-[#FF5C00]">*</span></label>
            <input value={name} onChange={(e) => setName(e.target.value)} required className={inputCls} placeholder="Ion Popescu" />
          </div>
          <div>
            <label className={labelCls}>Email <span className="text-[#FF5C00]">*</span></label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>Parolă <span className="text-[#FF5C00]">*</span></label>
            ...
            <div>
              <label className={labelCls}>Confirmă parola <span className="text-[#FF5C00]">*</span></label>
          ...
          <p className="text-[11px] text-[#6B6B68] leading-relaxed">
            Prin crearea contului accepți <a href="/termeni" className="text-[#0A0A0A] underline hover:text-[#FF5C00]">Termenii</a> și <a href="/confidentialitate" className="text-[#0A0A0A] underline hover:text-[#FF5C00]">Politica de confidențialitate</a>.
          </p>
        </div>
      )}
      ...
      <div className="flex gap-2 pt-2">
        {step > 1 && (
          <button type="button" onClick={() => { setError(''); setStep(step - 1); }} className="px-5 py-3 bg-white border border-[#E8E8E4] hover:border-[#0A0A0A] text-[#0A0A0A] font-medium rounded-xl text-[14px] transition-colors">
            Înapoi
          </button>
        )}
        <button type="submit" disabled={loading} className="flex-1 py-3 bg-[#FF5C00] hover:bg-[#E04E00] disabled:bg-[#FF5C00]/60 disabled:cursor-not-allowed text-white font-semibold rounded-xl text-[14px] transition-colors">
      ...
      <p className="text-center text-[13px] text-[#6B6B68]">
        Ai deja cont?{' '}
        <a href="/auth/login" className="text-[#0A0A0A] font-semibold hover:text-[#FF5C00] transition-colors">Autentifică-te</a>
      </p>
    </form>
  );
}
