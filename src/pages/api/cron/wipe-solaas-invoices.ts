// TEMP — wipe ALL invoices for the solaastech test company (local only). CRON_SECRET. DELETE after use.
...
export const GET: APIRoute = async ({ request }) => {
  ...
  const rows = await db.select({ id: transportInvoices.id, fullNumber: transportInvoices.fullNumber })
    .from(transportInvoices).where(eq(transportInvoices.companyId, u.companyId));
