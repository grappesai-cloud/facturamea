...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (readCookie(request.headers.get('cookie'), 'fm_oauth_native') === '1') {
      ...
      try { await logAction({ userId, companyId, action: 'auth.oauth_apple_native', request }); } catch {}
      const h = new Headers({ Location: `${NATIVE_SCHEME}://auth?token=${encodeURIComponent(signNativeAuth(userId))}` });
      h.append('Set-Cookie', 'fm_a_state=; Path=/; HttpOnly; SameSite=None; Secure; Max-Age=0');
      h.append('Set-Cookie', 'fm_oauth_native=; Path=/; HttpOnly; SameSite=None; Secure; Max-Age=0');
      h.append('Set-Cookie', 'fm_oauth_fe=; Path=/; HttpOnly; SameSite=None; Secure; Max-Age=0');
